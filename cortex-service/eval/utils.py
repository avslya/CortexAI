...
import docker
from eval import cortex_ENGINE_API_URL, EvalSettings
from src.core.engine import add_quotes
from src.providers.engine.cortex import cortexEngine

load_dotenv(".env", override=True)


async def get_data_from_cortex_engine(
    sql: str,
    mdl_json: dict,
    api_endpoint: str,
...
async def get_contexts_from_sql(
    sql: str,
    mdl_json: dict,
    api_endpoint: str = cortex_ENGINE_API_URL,
    timeout: float = 300,
    **kwargs,
) -> list[str]:
...
def engine_config(
    ...
    if isinstance(engine, cortexEngine):
...
def replace_cortex_engine_env_variables(engine_type: str, data: dict, config_path: str):
