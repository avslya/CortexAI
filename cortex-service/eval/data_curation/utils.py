...
cortex_IBIS_ENDPOINT = os.getenv("cortex_IBIS_ENDPOINT", "http://localhost:8000")
cortex_ENGINE_ENDPOINT = os.getenv("cortex_ENGINE_ENDPOINT", "http://localhost:8080")
        ...
        for question_sql_pair in question_sql_pairs:
            task = asyncio.ensure_future(
                is_sql_valid(
                    ...
                    mdl_json,
                    connection_info,
                    cortex_ENGINE_ENDPOINT
                    if data_source == "duckdb"
                    else cortex_IBIS_ENDPOINT,
                )
            )
            tasks.append(task)
...
async def get_contexts_from_sqls(
    sqls: list[str],
    mdl_json: dict,
    api_endpoint: str = cortex_ENGINE_ENDPOINT,
    timeout: float = TIMEOUT_SECONDS,
) -> list[list[str]]:
    async with aiohttp.ClientSession():
        tasks = []
...
""",
    ...
    try:
        ...
        sqls_data = await get_data_from_cortex_engine_with_sqls(
            sqls,
            data_source,
            mdl_json,
            connection_info,
...
async def get_data_from_cortex_engine_with_sqls(
    sqls: List[str],
    data_source: str,
    mdl_json: dict,
    connection_info: dict,
