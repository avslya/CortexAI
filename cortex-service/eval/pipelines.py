...
class RetrievalPipeline(Eval):
    ...
    def metrics(engine_info: dict) -> dict:
        cortex_engine_info = engine_info.copy()
        cortex_engine_info["api_endpoint"] = cortex_ENGINE_API_URL

        return {
            "metrics": [
                ContextualRecallMetric(engine_info=cortex_engine_info),
                ContextualRelevancyMetric(),
                ContextualPrecisionMetric(),
            ]
        }
    ...
    def metrics(
        ...
        return {
            "metrics": [
                ...
                AnswerRelevancyMetric(engine_info=cortex_engine_info),
                FaithfulnessMetric(engine_info=cortex_engine_info),
                ExactMatchAccuracy(),
                ExecutionAccuracy(),
