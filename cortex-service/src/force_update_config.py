...
# so we need to force update the engine names to the correct ones when we would like to use cortex UI
            ...
            for pipe in doc.get("pipes", []):
                if "engine" in pipe:
                    if pipe["name"] == "sql_functions_retrieval":
                        pipe["engine"] = "cortex_ibis"
                    else:
                        pipe["engine"] = "cortex_ui"

    # Write back to the file
    with open("config.yaml", "w") as file:
        yaml.safe_dump_all(documents, file, default_flow_style=False)

    print("Successfully updated engine names to 'cortex_ui' in all pipelines")
