...
logger = logging.getLogger("cortex-ai-service")


@provider("cortex_ui")
class cortexUI(Engine):
    def __init__(
        self,
        endpoint: str = os.getenv("cortex_UI_ENDPOINT"),
        **_,
    ):
...
@provider("cortex_ibis")
class cortexIbis(Engine):
    def __init__(
        self,
        endpoint: str = os.getenv("cortex_IBIS_ENDPOINT"),
        source: str = os.getenv("cortex_IBIS_SOURCE"),
        manifest: str = os.getenv("cortex_IBIS_MANIFEST"),
        connection_info: str = os.getenv("cortex_IBIS_CONNECTION_INFO"),
        **_,
    ):
        self._endpoint = endpoint
        self._source = source
        ...
        **kwargs,
        ...
        try:
            async with session.post(
                api_endpoint,
                headers={
                    "x-cortex-fallback_disable": "false" if allow_fallback else "true",
                },
                json={
                    "sql": sql,
                    "manifestStr": self._manifest,
...
@provider("cortex_engine")
class cortexEngine(Engine):
    def __init__(
        self,
        endpoint: str = os.getenv("cortex_ENGINE_ENDPOINT"),
        manifest: str = os.getenv("cortex_ENGINE_MANIFEST"),
        **_,
    ):
        self._endpoint = endpoint
        self._manifest = manifest
