...
export default async function handler(
  ...
  try {
    ...
    // Step 1: Generate SQL
    const askTask = await cortexAIAdaptor.ask({
      query: question,
    ...
    while (true) {
      askResult = await cortexAIAdaptor.getAskResult(askTask.queryId);
    ...
    if (askResult.error) {
      const errorMessage =
        (askResult.error as cortexAIError).message || 'Unknown error';
    ...
    if (askResult.type === AskResultType.GENERAL) {
      ...
      let explanation = '';
      const stream = await cortexAIAdaptor.getAskStreamingResult(askTask.queryId);
    ...
    const summaryTask =
      await cortexAIAdaptor.createTextBasedAnswer(textBasedAnswerInput);
    ...
    while (true) {
      summaryResult = await cortexAIAdaptor.getTextBasedAnswerResult(
        summaryTask.queryId,
      );
      if (
    ...
    if (summaryResult.status === TextBasedAnswerStatus.SUCCEEDED) {
      const stream = await cortexAIAdaptor.streamTextBasedAnswer(
        summaryTask.queryId,
      );
