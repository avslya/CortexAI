...
import { AskResult, cortexAILanguage } from '@/apollo/server/models/adaptor';
...
export default async function handler(
  ...
  try {
    ...
    const histories = threadId
      ...
      : undefined;
    const task = await cortexAIAdaptor.ask({
      query: question,
    ...
    while (true) {
      result = await cortexAIAdaptor.getAskResult(task.queryId);
    ...
    if (returnSqlDialect && sql) {
      ...
      if (project.type === DataSourceName.DUCKDB) {
        nativeSql = await cortexEngineAdaptor.getNativeSQL(sql, {
