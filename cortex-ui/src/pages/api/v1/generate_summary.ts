...
const { projectService, cortexAIAdaptor, deployService, queryService } =
  components;
...
export default async function handler(
  ...
  try {
    ...
    while (true) {
      result = await cortexAIAdaptor.getTextBasedAnswerResult(task.queryId);
      if (
    ...
    if (result.status === TextBasedAnswerStatus.SUCCEEDED) {
      const stream = await cortexAIAdaptor.streamTextBasedAnswer(task.queryId);
