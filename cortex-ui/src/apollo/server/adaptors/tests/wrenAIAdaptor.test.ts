...
import { cortexAIAdaptor } from '../cortexAIAdaptor';
import {
...
describe('cortexAIAdaptor', () => {
  const baseEndpoint = 'http://test-endpoint';
  let adaptor: cortexAIAdaptor;

  beforeEach(() => {
    adaptor = new cortexAIAdaptor({ cortexAIBaseEndpoint: baseEndpoint });
    jest.clearAllMocks();
  });
