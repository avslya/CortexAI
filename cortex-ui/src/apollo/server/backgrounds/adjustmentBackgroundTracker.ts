...
import { IcortexAIAdaptor } from '../adaptors';
import { TelemetryEvent, cortexService } from '../telemetry/telemetry';
  ...
  implements IAdjustmentBackgroundTaskTracker
{
  ...
  public async createAdjustmentTask(
    ...
    try {
      ...
      const response = await this.cortexAIAdaptor.createAskFeedback(input);
  ...
  public async rerunAdjustmentTask(
    ...
    const response = await this.cortexAIAdaptor.createAskFeedback({
      ...input,
  ...
  public async cancelAdjustmentTask(queryId: string): Promise<void> {
    await this.cortexAIAdaptor.cancelAskFeedback(queryId);
      ...
      ([queryId, task]) =>
        async () => {
          try {
            ...
            const result =
              await this.cortexAIAdaptor.getAskFeedbackResult(queryId);
            task.lastPolled = now;
