...
import { IcortexAIAdaptor } from '@server/adaptors';
...
export class InstructionService implements IInstructionService {
  ...
  private readonly cortexAIAdaptor: IcortexAIAdaptor;
  constructor({
  ...
  public async createInstruction(
    ...
    try {
      ...
      const { queryId } = await this.cortexAIAdaptor.generateInstruction([
  ...
  async deleteInstruction(id: number, projectId: number): Promise<void> {
    ...
    try {
      ...
      await this.cortexAIAdaptor.deleteInstructions([id], instruction.projectId);
      await tx.commit();
    } catch (e: any) {
      await tx.rollback();
  ...
  private async waitDeployInstruction(
    ...
    let res = await this.cortexAIAdaptor.getInstructionResult(queryId);
    let retryCount = 0;

    while (!isFinalStatus(res.status) && retryCount < maxRetries) {
      await new Promise((resolve) => setTimeout(resolve, 1000));
      res = await this.cortexAIAdaptor.getInstructionResult(queryId);
      retryCount++;
    }
