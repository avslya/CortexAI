import { cortexAIDeployStatusEnum } from '@server/models/adaptor';
import { IcortexAIAdaptor } from '../adaptors/cortexAIAdaptor';
import {
  Deploy,
  DeployStatusEnum,
  ...
  public async deploy(manifest, projectId, force = false) {
    ...
    try {
      ...
      const { status: aiStatus, error: aiError } =
        await this.cortexAIAdaptor.deploy({
          manifest,
          hash,
        });
      ...
      const status =
        aiStatus === cortexAIDeployStatusEnum.SUCCESS
