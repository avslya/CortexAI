...
export class SqlPairService implements ISqlPairService {
  ...
  public async generateQuestions(
    ...
    try {
      ...
      const { queryId } = await this.cortexAIAdaptor.generateQuestions({
  ...
  public async createSqlPair(
    ...
    try {
      ...
      const { queryId } = await this.cortexAIAdaptor.deploySqlPair(
        projectId,
        newPair,
      );
    ...
    for (const pairs of chunks) {
      await Promise.allSettled(
        ...
        if (errorPairs.length > 0) {
          ...
          await tx.rollback();
          await this.cortexAIAdaptor.deleteSqlPairs(
            projectId,
  ...
  async deleteSqlPair(projectId: number, sqlPairId: number): Promise<boolean> {
    ...
    try {
      ...
      await this.cortexAIAdaptor.deleteSqlPairs(projectId, [sqlPairId]);
      await tx.commit();
      return true;
    } catch (error) {
  ...
  private async waitUntilSqlPairResult(
    ...
    let result = await this.cortexAIAdaptor.getSqlPairResult(queryId);
    while (!this.isFinishedState(result.status)) {
      await new Promise((resolve) => setTimeout(resolve, 500));
      result = await this.cortexAIAdaptor.getSqlPairResult(queryId);
    }
    return result;
  }
  ...
  private async waitQuestionGenerateResult(
    ...
    let result = await this.cortexAIAdaptor.getQuestionsResult(queryId);
    while (
      ...
      await new Promise((resolve) => setTimeout(resolve, 500));
      result = await this.cortexAIAdaptor.getQuestionsResult(queryId);
    }
    return result;
  }
