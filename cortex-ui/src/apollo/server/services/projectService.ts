...
import { IProjectRepository, cortex_AI_CONNECTION_INFO } from '../repositories';
...
export interface ProjectData {
  displayName: string;
  type: DataSourceName;
  connectionInfo: cortex_AI_CONNECTION_INFO;
}
...
export class ProjectService implements IProjectService {
  ...
  public async generateProjectRecommendationQuestions(): Promise<void> {
    ...
    const recommendQuestionResult =
      await this.cortexAIAdaptor.generateRecommendationQuestions({
        manifest,
  ...
  public async getProjectRecommendationQuestions() {
    ...
    if (project.queryId) {
      ...
      result.error = project.questionsError as cortexAIError;
    }
    return result;
  }
