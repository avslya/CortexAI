...
export class AskingTaskTracker implements IAskingTaskTracker {
  ...
  public async createAskingTask(
    ...
    try {
      ...
      const response = await this.cortexAIAdaptor.ask(input);
      ...
      if (
        ...
        const result = await this.cortexAIAdaptor.getAskResult(queryId);
  ...
  public async cancelAskingTask(queryId: string): Promise<void> {
    await this.cortexAIAdaptor.cancelAsk(queryId);
  }
