...
export class QueryService implements IQueryService {
  ...
  public async preview(
    ...
    if (this.useEngine(dataSource)) {
      if (dryRun) {
        logger.debug('Using cortex engine to dry run');
        await this.cortexEngineAdaptor.dryRun(sql, {
          manifest: mdl,
          limit,
        });
        return true;
      } else {
        logger.debug('Using cortex engine to preview');
        const data = await this.cortexEngineAdaptor.previewData(sql, mdl, limit);
