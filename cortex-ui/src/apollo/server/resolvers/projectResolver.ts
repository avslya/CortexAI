...
import { DuckDBPrepareOptions } from '@server/adaptors/cortexEngineAdaptor';
...
export class ProjectResolver {
  ...
  public async getSettings(_root: any, _arg: any, ctx: IContext) {
    ...
    return {
      productVersion: ctx.config.cortexProductVersion || '',
      dataSource: {
        type: dataSourceType,
        properties: {
  ...
  public async resetCurrentProject(_root: any, _arg: any, ctx: IContext) {
    ...
    try {
      ...
      await ctx.cortexAIAdaptor.delete(id);
  ...
  public async updateDataSource(
    ...
    if (dataSourceType === DataSourceName.DUCKDB) {
      // prepare duckdb environment in cortex-engine
  ...
  public async saveTables(
    ...
    try {
      ...
      // async deploy to cortex-engine and ai service
      this.deploy(ctx);
  ...
  private async buildDuckDbEnvironment(
    ...
    await ctx.cortexEngineAdaptor.prepareDuckDB({
      sessionProps: configurations,
      initSql: initSqlWithExtensions,
    } as DuckDBPrepareOptions);

    // check can list dataset table
    await ctx.cortexEngineAdaptor.listTables();

    // patch cortex-engine config
    const config = {
      'cortex.datasource.type': 'duckdb',
    };
    await ctx.cortexEngineAdaptor.patchConfig(config);
  }
}
