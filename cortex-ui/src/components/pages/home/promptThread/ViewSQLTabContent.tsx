...
export default function ViewSQLTabContent(props: AnswerResultProps) {
  ...
  const onCopy = () => {
    if (!nativeSQLResult.nativeSQLMode) {
      message.success(
        <>
          You copied cortex SQL. This dialect is for the cortex Engine and may not
  ...
  return (
    <div className="text-md gray-10 p-6 pb-4">
      <Alert
        ...
        message={
          <>
            You’re viewing cortex SQL by default. If you want to run this query on
            your own database, click “Show original SQL” to get the exact
            syntax.
            <Typography.Link
              className="underline ml-1"
              href="https://docs.getcortex.ai/oss/guide/home/cortex_sql"
              target="_blank"
              rel="noopener noreferrer"
            >
              Learn more about cortex SQL
            </Typography.Link>
          </>
        }
        type="info"
          ...
          <div>
            {nativeSQLResult.nativeSQLMode ? (
              ...
              <span className="d-flex align-center gx-2">
                <Logo size={18} />
                <Text className="gray-8 text-medium text-sm">cortex SQL</Text>
              </span>
            )}
          </div>
