...
# check if cortex.experimental-enable-dynamic-fields is set, otherwise append it with true
if ! grep -q "cortex.experimental-enable-dynamic-fields" ${data_path}/config.properties; then
  echo "cortex.experimental-enable-dynamic-fields is not set, set it to true"
  echo "cortex.experimental-enable-dynamic-fields=true" >>${data_path}/config.properties
fi
